...
use mesocarp::{
    comms::mailbox::ThreadWorldUser, logging::journal::Journal, scheduling::{htw::Clock, Scheduleable},
};

use crate::{
    agents::{AgentSupport, ThreadedAgent},
    messages::{AntiMsg, Msg, Transfer},
    st::{event::{Action, Event}, TimeInfo},
    SimError,
};
...
impl LocalTime {
    pub fn init(global_clock: Arc<AtomicU64>, horizon: u64, timestep: f64, terminal: f64) -> Self {
        Self {
            time: 0,
            horizon,
    ...
    > LP<SLOTS, CLOCK_SLOTS, CLOCK_HEIGHT, MessageType>
{
    pub fn init(
        ...
        let time = LocalTime::init(registry.0, horizon, timestep, terminal);
        let event_process = LocalEventSystem::new()?;
        let mail_process = LocalMailSystem::new(anti_msg_arena_size)?;
        let agent_id = registry.2;
        let supports = AgentSupport::new(Some(registry.1), Some(state_arena_size));
        Ok(Self {
            agent,
            agent_id,
            supports,
    ...
    fn commit(&mut self, event: Event) {
        ...
        if event_maybe.is_err() {
            self.event_process.overflow.insert(Reverse(event_maybe.err().unwrap()));
        }
    }
    ...
    fn commit_mail(&mut self, msg: Msg<MessageType>) {
        ...
        if msg.is_err() {
            self.mail_process.overflow.insert(Reverse(msg.err().unwrap()));
        }
    }
    ...
    fn annihilate(&mut self, anti_msg: AntiMsg) {
        ...
        for k in 0..CLOCK_HEIGHT {
            ...
            if diff >= startidx {
                if diff >= (((CLOCK_SLOTS).pow(1 + CLOCK_HEIGHT as u32) - CLOCK_SLOTS) / (CLOCK_SLOTS - 1)) {
                    break;
                }
                if diff > endidx {
                    continue;
                ...
                let offset =
                    ((diff - startidx) / (SLOTS.pow(k as u32)) + idxs[k]) % SLOTS;
        ...
        self.mail_process.overflow = BTreeSet::from_iter(vec.into_iter());
    }
    ...
    pub fn step(&mut self) -> Result<(), SimError> {
        ...
        // step forward
        if let Some(msgs) = self.mail_process.schedule.tick().ok() {
            for msg in msgs {
        ...
        if let Some(events) = self.event_process.local_clock.tick().ok() {
            for event in events {
        ...
        self.event_process.local_clock.increment(&mut self.event_process.overflow);
        self.time.time += 1;
        Ok(())
    }
